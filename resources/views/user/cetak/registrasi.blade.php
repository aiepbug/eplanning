...
<?php 
	...
	$no=1;
?>
<div class="kanan"><img src="data:../download;base64, {{ DNS1D::getBarcodePNG('$reg','C39+') }}"/>
<div>{{$reg}}</div>
</div>
...
<table border="0">
	...
	<tr><td>No. Registrasi</td><td>: {{ $reg }}</td></tr>
	<tr><td>No. Identitas KTP</td><td>: {{ $bio->nik }}</td></tr>
	<tr><td>Nama</td><td>: {{ $bio->nama }}</td></tr>
	<tr><td>Tempat/Tanggal lahir</td><td>: {{ $bio->tempat_lahir }} / {{ date("d-m-Y",strtotime($bio->tanggal_lahir)) }}</td></tr>
	<tr><td>Jenis kelamin</td><td>: @if($bio->jender=='L') Pria @elseif($bio->jender=='P') Wanita @endif</td></tr>
	<tr><td>Alamat</td><td>: {{ $bio->alamat }}</td></tr>
	<tr><td>No. HP</td><td>: {{ $bio->hp }}</td><td></td></tr>
	<tr><td>Formasi dilamar</td><td>: {{ $formasi->nama_formasi }}</td><td></td></tr>
	<tr><td>Kualifikasi akademik</td><td colspan="2">: 
		<table width="100%" class="" border="1">
			...
			<tr class="tengah"><td>{{ $no++ }}</td><td>{{ strtoupper($a->jenjang) }}</td><td class="kiri">{{ $a->prodi }}</td><td class="kiri">{{ $a->pt }}</td></tr>
			@endforeach
		</table>
	</td></tr>
...
<table>
<tr>
	<td width="60%" class="justi"></td><td width="10%"></td><td width="30%">............, {{ strftime("%d %B %Y")}}</td></tr>
...
<tr><td></td><td></td><td>{{ $bio->nama }}<br>(TANDA TANGAN) *</td></tr>
</table>
<br><br><br>
<p class="kecil">
